...
def test_deal_hand():
    ...
    for k in hand.keys():
        if (not type(k) is str) or (not type(hand[k]) is int):
            print "FAILURE: test_deal_hand()"
            print "\tUnexpected type of dictionary: \
            string -> int expected, but was", type(k), "->", type(hand[k])
    ...
    if repeats > 10:
        print "FAILURE: test_deal_hand()"
        print "\tSame hand returned", repeats,
        "times by deal_hand(). This is HIGHLY unlikely."
